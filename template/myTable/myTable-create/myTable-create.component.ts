...
// import { myTableRestapiTestComponent } from '../myTable-restapi-test/myTable-restapi-test.component';
...
@Component({
    selector: 'myTable-create',
    templateUrl: './myTable-create.component.html',
    styleUrls: ['./myTable-create.component.less'],
    providers: [FormBuilder]
})
export class MyTableCreateComponent implements OnInit {

    myTableName: string = "";//连接配置名称

    title: Array<any>;
    myTableTypeList: any;//// 连接配置类型
    singledom: any = "JDBC";
    single: any;
    versions: any;//ES版本号
    ...
    dataURLprefix: any;
    url: any;
    // @ViewChild(myTableRestapiTestComponent)
    // private create: myTableRestapiTestComponent;
    ...
    constructor(
        ...
        private myTableCreateService: MyTableCreateService,
    ) {
        this.state = true;
        this.single = "JDBC";
        this.singledom = "JDBC";
        this.myTableTypeList = dictionary.dataSourceType;
        ...
        if (this.route.snapshot.params.id !== undefined) {
            this.title[1]['url'] = `myTable/detail/${this.route.snapshot.params.id}`;
            this.title[2]['url'] = `myTable/${this.route.snapshot.url[0]['path']}/${this.route.snapshot.params.id}`;
        } else {
            this.title[1]['url'] = `myTable/${this.route.snapshot.url[0]['path']}`;
        }//面包屑
        ...
        if (this.route.snapshot.params.id == undefined) {
            ...
            this.validateForm = this.fb.group({
                myTableName: ['', [Validators.required, this.folderNameAsyncValidator]],
                myTableType: ['JDBC', [Validators.required]],
            ...
            this.validateForm1 = this.fb.group({
                myTableName: ['', [Validators.required, this.folderNameAsyncValidator]],
                myTableType: ['RESTAPI', [Validators.required]],
                state: ['true', [Validators.required]],
                myTableMatter: [''],
                // res
            ...
            this.validateForm2 = this.fb.group({
                myTableName: ['', [Validators.required, this.folderNameAsyncValidator]],
                myTableType: ['ELASTICSEARCH', [Validators.required]],
                state: ['true', [Validators.required]],
                myTableMatter: [''],
                //ELASTICSEARCH
            ...
            this.validateForm3 = this.fb.group({
                myTableName: ['', [Validators.required, this.folderNameAsyncValidator]],
                myTableType: ['SPARK', [Validators.required]],
                state: ['true', [Validators.required]],
                myTableMatter: [''],
                //  "SPARK"
                dataIP: ['', [Validators.required, this.folderIpAsyncValidator]],
            ...
            this.disabled = true;
            this.myTableCreateService.get(this.route.snapshot.params.id).then((response: any) => {
                ...
                this.myTableName=response.name;
                this.validateForm = this.fb.group({
                    myTableName: [response.name, [Validators.required, this.folderNameAsyncValidator]],
                    myTableType: ['JDBC', [Validators.required]],
                    state: ['true', [Validators.required]],
                    myTableMatter: [response.description],
                    //  "JDBC"
    ...
    testTheConnection() {
        ...
        if (this.singledom == "ELASTICSEARCH") {
            obj = {
                "name": value2.myTableName,
                "type": value2.myTableType,
                "version": value2.versionNumber,
                "url": value2.dataURLprefix + value2.dataURL,
                "status": value2.state,
                "description": value2.myTableMatter
            }
        }
        if (this.singledom == "JDBC") {
            obj = {
                "name": value.myTableName,
                "type": value.myTableType,
                "host": value.dataIP,
                "port": value.port,
                ...
                "status": value.state,
                "description": value.myTableMatter
            }
        }
        if (this.singledom == "RESTAPI") {
            obj = {
                "name": value1.myTableName,
                "type": value1.myTableType,
                "url": value1.dataURLprefix + value1.dataURLres,
                "description": value1.myTableMatter,
                "status": value1.state
            }
        }
        if (this.singledom == "SPARK") {
            obj = {
                "name": value3.myTableName,
                "type": value3.myTableType,
                "host": value3.dataIP,
                "port": value3.port,
                ...
                // 
                "status": value3.state,
                "description": value3.myTableMatter,
                "sparkDatabase":value3.databasewarehouse
            }
        }
        if (this.route.snapshot.params.id !== undefined) {
            obj["id"] = this.route.snapshot.params.id;
            this.myTableCreateService.test(obj).then(response => {
                if (response.success) {
                    return this._message.success(response.msg, "", this.myTableCreateService.msg['notification']['success'])
                } else {
                    return this._message.error(response.msg, "", this.myTableCreateService.msg['notification']['error'])
                }
            }).catch((err:any) => {
                this._message.error("错误", err);
            });
    ...
    saveTheDataSource(sure: any) {
        this.myTableCreateService.hasSaved = true;
        let value = this.validateForm.value;
        let value1 = this.validateForm1.value;
        let value2 = this.validateForm2.value;
        let value3 = this.validateForm3.value;
        ...
        if (this.route.snapshot.params.id !== undefined) {
            ...
            this.myTableCreateService.update(objsure).then(response => {
                if (JSON.parse(response._body).success) {
                    this.router.navigate(['/main/myTable/detail', this.route.snapshot.params.id]);
                    return this._message.success(JSON.parse(response._body).msg, "", this.myTableCreateService.msg['notification']['success']);
                } else {
                    return this._message.error(JSON.parse(response._body).msg, "", this.myTableCreateService.msg['notification']['error'])
                }
            ...
            this.myTableCreateService.create(objsure).then(response => {
                if (response.success) {
                    this.router.navigate(['/main/myTable']);
                    return this._message.success(response.msg, "", this.myTableCreateService.msg['notification']['success'])
                } else {
                    return this._message.error(response.msg, "", this.myTableCreateService.msg['notification']['error'])
                }
